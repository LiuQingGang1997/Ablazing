...
const startIdx = code.indexOf('const buildProducts =');
const endIdx = code.indexOf('const [activeBrandId, setActiveBrandId]');
...
const beforeCode = code.substring(0, startIdx);
const afterCode = code.substring(endIdx);

const newCode = beforeCode + mockDataCode + '\n  ' + afterCode;

fs.writeFileSync('src/pages/HotStores.tsx', newCode);
console.log('Successfully replaced static code block.');
